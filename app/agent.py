...
async def run_scaffold_agent_kimi(
    ...
    prompt = (
        ...
        f"DESIGN TOKENS (required, enables live theme editing):\n"
        f"- Put ALL theme values on `:root` as CSS custom properties at the "
        f"TOP of styles.css. The viewer exposes an editor that mutates these "
        f"at runtime, so every color / space / radius / font-size in your "
        f"scaffold MUST reference a `var(--token-name)` — no hardcoded hex, "
        f"px, or rem values in component rules.\n"
        f"- Required token groups (emit ALL of them, even if some go unused):\n"
        f"    Colors: --color-bg, --color-surface, --color-text, "
        f"--color-text-muted, --color-primary, --color-primary-fg, "
        f"--color-accent, --color-border\n"
        f"    Space: --space-1 through --space-8 (4/8/12/16/24/32/48/64 px)\n"
        f"    Radius: --radius-sm, --radius-md, --radius-lg, --radius-pill\n"
        f"    Type: --font-sans, --font-mono, --text-xs, --text-sm, "
        f"--text-base, --text-lg, --text-xl, --text-2xl, --text-3xl, "
        f"--text-4xl\n"
        f"- Also emit a `@media (prefers-color-scheme: dark)` block that "
        f"overrides the color tokens. Dark mode must Just Work.\n"
        f"- NEVER write `color: #0f172a` — write `color: var(--color-text)`. "
        f"The token editor can't help the user if values are hardcoded.\n\n"
        f"PWA / OFFLINE (required — differentiates our scaffolds):\n"
        f"- Emit `manifest.webmanifest` with name, short_name, start_url (.), "
        f"display 'standalone', background_color + theme_color pulled from "
        f"your token palette, and at least one inline SVG icon (data URL "
        f"or file). Link it from index.html: `<link rel=\"manifest\" "
        f"href=\"manifest.webmanifest\">`.\n"
        f"- Emit `sw.js` — a small service worker that precaches "
        f"index.html, styles.css, app.js (plus any extra pages/assets) on "
        f"install and serves cache-first on fetch. Keep it ~30 lines. "
        f"Register it from app.js: `if ('serviceWorker' in navigator) "
        f"navigator.serviceWorker.register('./sw.js').catch(()=>{{}});`.\n"
        f"- Add `<meta name=\"theme-color\" content=\"...\">` to index.html "
        f"matching manifest theme_color. Also include `<meta name=\"viewport\" "
        f"content=\"width=device-width,initial-scale=1\">`.\n"
        f"- This makes the scaffold installable + offline-capable with zero "
        f"user effort. Don't skip it."
    )
